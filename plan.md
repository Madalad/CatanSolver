...
- **↩ Revisit from Phase 5 (second-player settlement ordering):** going second, *which* of the two settlements is placed second sets the opening hand (one card per adjacent producing hex). The opening advisor/practice win-% engine (`node_score` + gap model) is **hand-blind**, so for now the two spots are shown unordered (just the model-answer star) with a "order isn't a ranking" note (no ordering optimised). **Phase 6's full-game bot is hand-aware** — once it can evaluate a position *with* resources in hand, wire it in to choose the better placement order (evaluate both orderings, pick the higher win-%) and surface the recommended order. This is the natural home for the feature; doing it earlier would have meant a throwaway bespoke heuristic.

- **↩ Revisit from Phase 5 (opening win-% fidelity):** the displayed opening win-% is a *static* read — the heuristic opening-strength **gap** mapped through a logistic (`opening_gap_model.json`) fit on **WeightedRandom** self-play outcomes (advisor-level verified to give the same curve → opening leverage is skill-independent). It captures opening *production* strength, not real play (no dice/robber/dev cards/tactics), and inference plays **no games** (that's why it's instant, ~10 ms — essential for per-pick annotation). Once Phase 6's strong in-process bot exists, two upgrades become available: **(B, recommended, low-risk)** re-fit the gap (or value-model) → win-% map on **AdvisorPlayer** self-play outcomes — better-grounded numbers, still instant at inference; **(A, optional)** an on-demand "deep evaluate this position" button that plays out N equal-skill games with the strong bot for a true playout estimate — far more faithful but **seconds–minutes** per position (each strong-bot game is seconds), so not viable for live per-pick annotation.
